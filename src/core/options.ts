...
export type Options = {
	 ...
	 *
	 * - 'disabled': Never cache any images. Force image re-export even if an
	 *   existing image in the `cacheDirectory` appears unchanged from the source
	 *   in Photos.app. This will be slow!
	 * - 'enabled': Use the cached image, but check for changes in Photos.app.
	 * - 'aggressive': Use the first cached image forever (Fastest, may be outdated,
	...
	/**
	 * Experimental mode persists a single aphex-swift session for all requests,
	...
	/**
	 * Maximum number of concurrent image exports. Lower values reduce load on
	 * Photos.app and system resources.
	 *
	 * @default 4
	 */
	maxConcurrentExports?: number
	/**
	 * Additional image processing options. (Resizing, color profile enforcement,
	 * etc.) Sensible defaults provided if undefined.
	 */
	processOptions?: PartialDeep<ExportOptions['processOptions']>
	/**
	 * Delete all files in `cacheDirectory` that are not in the exported image set
	 * at the end of a production build. Skipped in dev because the dev server
	 * only sees imports for routes the user actually visits, so pruning there
	 * would delete cache entries for unvisited pages.
	 *
	 * @default false
	 */
	pruneCacheOnBuild?: boolean
	/**
	 * Returns an object with some extra details on the image instead of a plain
	 * path string. Occasionally useful for integration with other platforms.
	 *
	 * @default false
	 */
	returnMetadata?: boolean
	/**
	 * Require certain metadata fields to be present in the source files. This is
	 * highly opinionated and not currently configurable.
	 *
	 * @default false
	 */
